...
1. **Input 1**: The baseline model (BM) to be extended is given in [`examples/input/BooleanTcell.xlsx`](examples/input/BooleanTcell.xlsx), with 62 elements, key information of this model is:
 ...
 **Input 2**: The candidate event (CE) set, represented as a set of directed edges, is given in [`examples/input/MachineReadingOutput.csv`](examples/input/MachineReadingOutput.csv). The studied candidate set has 118 events, with key information as follows:
